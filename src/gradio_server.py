...
from douban_books_client import DoubanNewBooksClient
...
douban_new_books_client = DoubanNewBooksClient()
...
def generate_douban_report(model_type, model_name, repo, days):
    config.llm_model_type = model_type
    ...
    markdown_file_path = douban_new_books_client.export_new_books()
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    with gr.Tab("豆瓣最新图书"):
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_douban_report, inputs=[model_type, model_name,], outputs=[markdown_output, file_output])



if __name__ == "__main__":
    demo.launch(server_name="127.0.0.1")  # 启动界面并设置为公共可访问
    # 可选带有用户认证的启动方式
